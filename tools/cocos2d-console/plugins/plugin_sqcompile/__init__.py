...
'''
"sqcompile" plugin for cocos command line tool
'''
...
#import cocos
class CCPluginSqCompile(cocos.CCPlugin):
    """
    compiles (encodes) and minifies squirrel files
    """
    @staticmethod
    def plugin_name():
        return "sqcompile"
    ...
    def brief_description():
        ...
        return "minifies and/or compiles squirrel files"
    ...
    def get_relative_path(self, sqfile):
        try:
            pos = sqfile.index(self._current_src_dir)
            if pos != 0:
                raise cocos.CCPluginError("cannot find src directory in file path.")

            return sqfile[len(self._current_src_dir)+1:]
        except ValueError:
            raise cocos.CCPluginError("cannot find src directory in file path.") 
    ...
    def get_output_file_path(self, sqfile):
        ...
        luac_filepath = ""
        # Unknow to remove 'c' 
        relative_path = self.get_relative_path(sqfile)+"c"
