...
class CCPluginCompile(cocos.CCPlugin):
    ...
    def _add_custom_options(self, parser):
        ...
        group = parser.add_argument_group("lua/js/sq project arguments")
        group.add_argument("--no-res", dest="no_res", action="store_true", help="Package without project resources.")
        group.add_argument("--compile-script", dest="compile_script", type=int, choices=[0, 1], help="Diable/Enable the compiling of lua/js/sq script files.")
    ...
    def compile_sq_scripts(self, src_dir, dst_dir):
        if not self._project._is_sq_project():
            return
    ...
    def get_engine_dir(self):
        ...
        if engine_dir is None:
            ...
            elif self._project._is_sq_project():
                engine_dir = os.path.join(proj_dir, "frameworks", "cocos2d-x")
            else:
...
#                need_reset_dir = True
    ...
    def build_mac(self):
        ...
        if self._project._is_script_project():
            ...
            if self._project._is_sq_project() and self._compile_script:
                # on iOS, only invoke luacompile when lua encrypt is specified
                self.backup_dir(script_src_dir)
                self.compile_sq_scripts(script_src_dir, script_src_dir)
                need_reset_dir = True
    ...
    def build_win32(self):
        ...
        if self._project._is_sq_project():
            self.compile_sq_scripts(output_dir, output_dir)
