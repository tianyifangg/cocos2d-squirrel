...
def native_name_from_type(ntype, underlying=False):
    ...
    if not underlying and kind == cindex.TypeKind.ENUM:
        ...
        arr_type = ntype.get_array_element_type()
        arr_size = ntype.get_array_size()
        arr_type_name = native_name_from_type(arr_type)
        if arr_type_name == INVALID_NATIVE_TYPE:
            return INVALID_NATIVE_TYPE
        return "%s[%d]" % (arr_type_name, arr_size)
    else:
        ...
        if ntype.kind == cindex.TypeKind.POINTER:
            ...
            if ntype.kind == cindex.TypeKind.RECORD:
                if decl.kind == cindex.CursorKind.CLASS_DECL:
                    nt.is_object = True
                elif decl.kind == cindex.CursorKind.STRUCT_DECL:
                    nt.is_object = True
...
#                    nt.param_types = [NativeType.from_string(string) for string in param_arr]

        # mark argument as not supported
        if nt.name == INVALID_NATIVE_TYPE:
            nt.not_supported = True

        if not nt.namespaced_name in NativeType.type_map:
            NativeType.type_map[nt.namespaced_name] = nt
    ...
    def from_string(displayname):
        ...
        if displayname in NativeType.type_map:
            return NativeType.type_map[displayname]

        if displayname.endswith("*"):
            real_name = displayname.replace("*", "")
            if real_name in NativeType.type_map:
                base = NativeType.type_map[real_name]
                nt = NativeType()
                nt.name = base.name + "*"
                nt.namespaced_name = base.namespaced_name + "*"
                nt.whole_name = nt.namespaced_name
                nt.namespace_name = base.namespace_name
                nt.is_pointer = True
                NativeType.type_map[displayname] = nt
                return nt
        ...
        tokens = nt.namespaced_name.split("::")
...
#        nt.name = nt.namespaced_name.split("::")[-1]
    ...
    @staticmethod
    def dict_replace_value_re(dict, real_key):
        result = real_key
        for (k, v) in dict.items():
            if k.startswith('@'):
                k = k[1:]
                match = re.match('.*' + k, result)
                if match:
    ...
    def from_native(self, convert_opts):
        ...
        if "scriptname" in convert_opts:
            keys.append(convert_opts["scriptname"])
    ...
    def to_string(self, generator):
        ...
        if conversions.has_key('native_types'):
            ...
            to_replace = NativeType.dict_replace_value_re(native_types_dict, self.namespaced_name)
            if to_replace:
                name = to_replace
            else:
    ...
    def to_script_type(self, generator):
        if generator.script_type == "squirrel":
            conversions = generator.config['conversions']
            if conversions.has_key('to_script_type'):
                to_script_type_dict = generator.config['conversions']['to_script_type']
                name = self.namespaced_name
                if NativeType.dict_has_key_re(to_script_type_dict, [name]):
                    return NativeType.dict_get_value_re(to_script_type_dict, [name])
    ...
    def get_type_name(self, generator, in_collection = False):
        tname = self.get_whole_name(generator)
        tname = tname.replace("*", "").replace("const ", "").replace("&", "").strip()
        if in_collection:
            if tname.startswith("std::vector<"):
                return tname[len("std::vector<"):tname.rfind(">")]
            elif tname.startswith("std::map<"):
                return tname[len("std::map<"):tname.rfind(">")]

        return tname

class NativeField(object):
    def __init__(self, cursor, visibility):
...
class NativeConstant(object):
    def __init__(self, cursor, visibility):
        ...
        if _cursor.kind == cindex.CursorKind.ENUM_DECL or _cursor.kind == cindex.CursorKind.ENUM_CONSTANT_DECL:
            self.type.is_enum = True
    ...
    def to_native(self, convert_opts):
        ...
        if self.type.canonical_type != None:
            keys.append(self.type.canonical_type.name)
        keys.append(self.type.name)

        to_native_dict = generator.config['conversions']['to_native_var']
        ...
        if NativeType.dict_has_key_re(to_native_dict, keys):
            ...
            tpl = Template(tpl, searchList=[convert_opts])
            return str(tpl).rstrip()
        return "#pragma warning NO CONVERSION TO NATIVE FOR " + self.name + " : " + str(self.type.name)

# TODO:
#        if visibility == cindex.AccessSpecifierKind.PUBLIC and not self.name.startswith('_'):
#            print("  " + self.name)
...
class NativeFunction(object):
    ...
    def generate_code(self, current_class=None, generator=None, is_override=False):
        ...
        if gen.script_type == "spidermonkey":
            ...
            if (gen.script_type == "lua" or gen.script_type == "squirrel") and current_class != None :
...
class NativeOverloadedFunction(object):
    ...
    def generate_code(self, current_class=None, is_override=False):
        ...
        if current_class != None:
            if gen.script_type == "lua" or gen.script_type == "squirrel":
...
class NativeClass(object):
    ...
    def __init__(self, cursor, generator):
        ...
        if cursor.kind == cindex.CursorKind.STRUCT_DECL:
            self._current_visibility = cindex.AccessSpecifierKind.PUBLIC
        else:
    ...
    def public_fields(self):
        ...
        for field in self.fields:
            if field.visibility == cindex.AccessSpecifierKind.PUBLIC and not field.name.startswith('_'):
                should_skip = self.generator.should_skip(self.class_name, field.name)
                if not should_skip:
                    ret.append(field)
        return ret

    def public_constants(self):
        '''
        list of public constants
        '''
        ret = []
        for constant in self.constants:
            if constant.visibility == cindex.AccessSpecifierKind.PUBLIC and not constant.name.startswith('_'):
                ret.append(constant)
        return ret

    def public_enums(self):
        '''
        list of public enums
        '''
        ret = {}
        for name, enum_list in self.enums.iteritems():
            elist = []
            for en in enum_list:
                if en.visibility == cindex.AccessSpecifierKind.PUBLIC and not en.name.startswith('_'):
                    elist.append(en)
    ...
    def set_class_attributes(self):
        ...
        if self.is_ref_class:
            if not self.namespaced_class_name in NativeClass.all_ref_classes:
                NativeClass.all_ref_classes.append(self.namespaced_class_name)

    def get_all_parents_name(self):
        names = []
        for parent in self.parents:
            names.append(parent.namespaced_class_name)
            pnames = parent.get_all_parents_name()
            names.extend(pnames)
    ...
    def generate_code(self):
        ...
        apidoc_classhead_script = Template(file=os.path.join(self.generator.target,
                                       ...
                                       searchList=[{"current_class": self}])
        if self.generator.script_type == "lua" or self.generator.script_type == "squirrel":
            if self.generator.script_type == "squirrel":
                sufix = ".nut"
            else:
                sufix = ".lua"

            docfuncfilepath = os.path.join(self.generator.outdir + "/api", self.class_name + sufix)
        ...
        register = Template(file=os.path.join(self.generator.target, "templates", "register.c"),
                            searchList=[{"current_class": self}])
        register.all_ref_classes = NativeClass.all_ref_classes
    ...
    def _process_node(self, cursor):
        ...
        if cursor.kind == cindex.CursorKind.CXX_BASE_SPECIFIER:
            ...
            self.fields.append(NativeField(cursor, self._current_visibility))
            ...
            if not cursor.type.is_function_variadic() or self.generator.script_type == "squirrel":
                # skip if variadic
            ...
            if cursor.type.is_const_qualified() or cursor.type.get_pointee().is_const_qualified():
                self.constants.append(NativeConstant(cursor, self._current_visibility))

        elif cursor.kind == cindex.CursorKind.ENUM_DECL:
            enum_name = cursor.displayname
            for enum_node in cursor.get_children():
                if enum_node.kind == cindex.CursorKind.ENUM_CONSTANT_DECL:
                    nconstant = NativeConstant(enum_node, self._current_visibility)
                    if enum_name == "":
                        self.constants.append(nconstant)
                    else:
                        if not enum_name in self.enums:
                            self.enums[enum_name] = []
                        self.enums[enum_name].append(nconstant)
        # else:
...
class Generator(object):
    ...
    def generate_code(self):
        ...
        elif self.script_type == "squirrel":
            docfilepath = os.path.join(docfiledir, self.out_file + "_api.nut")
        else:
        ...
        self.head_file.write(str(layout_h))
        self.impl_file.write(str(layout_c))
        if self.script_type == "lua" or self.script_type == "squirrel":
...
#                        self.generated_classes[cursor.displayname] = nclass
    ...
    def _generate_class_code(self):
        for k in self.all_classes:
            nclass = self.all_classes[k]
            nclass.set_class_attributes()

        for k in self.all_classes:
            nclass = self.all_classes[k]
            nclass.generate_code()
            self.generated_classes[k] = nclass

    def scriptname_from_native(self, namespace_class_name, namespace_name, is_enum=False):
        ...
        if to_replace:
            namespace_class_name = to_replace
        ...
        for (k, v) in script_ns_dict.items():
            if k == namespace_name:
                return namespace_class_name.replace(k, v)
        ...
        if namespace_class_name.find("::") >= 0:
            if namespace_class_name.startswith("std::vector<"):
                ...
                return "std::vector<%s>" % self.scriptname_from_native(vector_class_name, vector_class_name[0:vector_class_name.rfind("::")+2])
            elif namespace_class_name.startswith("std::map<"):
                map_class_name = namespace_class_name[len("std::map<"):namespace_class_name.rfind(">")]
                return "std::map<%s>" % self.scriptname_from_native(map_class_name, map_class_name[0:map_class_name.rfind("::")+2])
            elif namespace_class_name.startswith("std::"):
    ...
    def scriptname_from_native_type(self, ntype):
        return self.scriptname_from_native(ntype.namespaced_name, ntype.namespace_name, ntype.is_enum)
